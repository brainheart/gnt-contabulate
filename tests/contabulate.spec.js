...
test.describe('Page Load', () => {
  test('loads and shows the Greek title', async ({ page }) => {
    await page.goto('/');
    await expect(page).toHaveTitle('Greek New Testament Tabular Explorer');
  });
...
test.describe('Segments Search', () => {
  ...
  test('can add commentator columns from the + popover', async ({ page }) => {
    ...
    expect(Number((await theophylact.locator('.count').textContent()).replace(/,/g, ''))).toBeGreaterThan(7000);
  ...
  test('count cells drill down and ancestor cells filter', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoaded(page);
    // Books → the 28 chapters of Matthew
    ...
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('starts with 01.Matt.');
    // Back un-drills
    await page.goBack();
    ...
    await expect(page.locator('#results tbody tr')).toHaveCount(27);
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('is New Testament');
  });
  ...
  test('vocabulary granularities put n-grams in the rows', async ({ page }) => {
    ...
    await page.fill('#q', 'φῶς');
    await page.locator('#addColumnBtn').click();
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('matches φῶς');
    const words = await page.locator('#results tbody tr td[data-key="ngram"]').allTextContents();
    expect(words.length).toBeGreaterThan(0);
    expect(words.every(w => /φῶς/iu.test(w))).toBeTruthy();
  });
...
test('vocabulary scope survives switching the n-gram size', async ({ page }) => {
  await page.goto('/?gran=word&s_ft_location=%5E01%5C.Matt%5C.');
  await waitForDataLoaded(page);
  await page.waitForSelector('#results tbody tr', { timeout: 10000 });
  await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('starts with 01.Matt.');
