...
const ELISION_RE = /['’ʼ᾽]/g;

function normalizeTerm(term) {
  return String(term || '').toLowerCase().normalize('NFC').replace(ELISION_RE, ' ').replace(/\s+/g, ' ').trim();
}
...
function tokenizeLineText(text) {
  const m = String(text || '').toLowerCase().normalize('NFC').replace(ELISION_RE, ' ').match(WORD_RE);
  return m ? m : [];
}
...
function buildHighlightRegexFromNgrams(ngrams) {
  ...
  for (const ng of ngrams) {
    ...
    parts.push("(?<![\\p{L}\\p{M}])" + tokens.join("(?:[^\\p{L}\\p{M}]|[ʼ᾽])+") + "(?=$|[^\\p{L}\\p{M}]|[ʼ᾽])");
  }
