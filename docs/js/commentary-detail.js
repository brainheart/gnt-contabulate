...
(function () {
  ...
  const COLUMNS = [
    { key: 'ref', label: 'HCF passage', sortable: true, defaultDir: 'asc' },
  ...
  async function openScope(scope, commentatorKey, expectedCount) {
    ...
    try {
      ...
      state.meta = describeComments(comments, commentatorLabel, expected, failedBooks) + " Passage labels use HCF numbering; comments are mapped by overlap with SBLGNT.";
      renderComments();
    } catch (error) {
      modal.title.textContent = state.title;
